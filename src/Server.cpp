...
{
	LOG_DEBUG << "Handling CGI connection: " << connection.m_clientSocket << " on fd: " << clientFd
 ...
 * 3. Connection::SendToCGI (out): connectionSendToCGI() sends the request body to the CGI process. It then changes the
 * state to Connection::ReceiveFromCGI.
 * 4. Connection::ReceiveFromCGI (in): connectionReceiveFromCGI() reads the response from the CGI process. If the
 * response is complete, it changes the state to Connection::BuildResponse.
...
bool isCompleteBody(Connection& connection)
{
	if (!connection.m_request.isChunked) {
		...
		if (contentLength < connection.m_buffer.size()) {
			...
			LOG_ERROR << "Content-Length: " << contentLength << "\nBuffer size: " << connection.m_buffer.size();
